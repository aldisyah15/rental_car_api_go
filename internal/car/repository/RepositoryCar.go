...
func (r CarRepository) GetCarById(id int) (*model.ResponseCar, error) {
	...
	query := `SELECT  id, name, brand, rental_price, image, horse_power, gear, description, stock, brand from detail_car where id = ?`
	err := r.db.QueryRow(query, id).Scan(&car.Id, &car.Name, &car.Brand, &car.RentalPrice, &rowsJsonImg, &car.Horsepower, &car.Gear, &car.Description, &car.Stock, &car.Logo)
